		...
		public static void generateWorld()
			{
				...
				// P.S. this should be a bell curve, so that its more likely to
				// start towards the center of the map
		...
		public static void newRoom(int x, int y, int depth)
			{
				...
				// System.out.println("\nGenerating Room (" + x + ", " + y + ")
				// with doorWeight of "+ getDoorWeight(depth + 1));
				...
				if (y != 0) {
					try {
						if (Map.map[x][y - 1] == null && Map.map[x][y].getDoors()[0] == "north") {
							// System.out.println("Moving North From (" + x + ",
							// " + y + ")");
				...
				if (x != 0) {
					try {
						if (Map.map[x - 1][y] == null && Map.map[x][y].getDoors()[1] == "west") {
							// System.out.println("Moving West From (" + x + ",
							// " + y + ")");
				...
				if (y != Map.map[0].length) {
					try {
						if (Map.map[x][y + 1] == null && Map.map[x][y].getDoors()[2] == "south") {
							// System.out.println("Moving South From (" + x + ",
							// " + y + ")");
							newRoom(x, y + 1, depth + 1);
						}
					} catch (Exception e) {
				...
				if (x != Map.map.length) {
					try {
						if (Map.map[x + 1][y] == null && Map.map[x][y].getDoors()[3] == "east") {
							// System.out.println("Moving East From (" + x + ",
							// " + y + ")");
							newRoom(x + 1, y, depth + 1);
						}
					} catch (Exception e) {
		...
		public static String generateRoomType(String[] doors)
			{
				...
				switch (closedDoors)
					...
					case 3:
						...
						String[] deadEndTypes = { "Dead End", "Treasure Room", "Closet", "Cellar", "Bunks", "Armory",
								"Office", "Prison" };
